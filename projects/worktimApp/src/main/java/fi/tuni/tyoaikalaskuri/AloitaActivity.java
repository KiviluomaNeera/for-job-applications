 ...
 */
public class AloitaActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_aloita);
    ...
    public void onStartClick(View view) {
        ...
        // Check if chosen task is "Auraus" or "Hiekoittaminen"
        ...
        if ((taskId == 1 || taskId == 2) && isLocationOK) {
            ...
            // Add new work to cloud and update users work id
            addWorkToCloud(newWork);
            addUserToCloud(currentUser);

            // Intent to take user back to MainWindowActivity
            Intent startIntent = new Intent(AloitaActivity.this, MainWindowActivity.class);
            startIntent.putExtra(EXTRA_MESSAGE, userId);
            startActivity(startIntent);
        } else if (taskId != 1 && taskId != 2) {
            // Oyherwise the same without adding location to work because
            // it is not needed
            newWork = new Work(nextWorkId, currentUser.getId(), currentUser.getName(),
                    taskId, taskName, clientId, clientName);
            currentUser.setCurrentWorkId(nextWorkId);
    ...
    public void onCancelClick(View view) {
        // Intent to take user back to MainWindowActivity
        Intent cancelIntent = new Intent(AloitaActivity.this, MainWindowActivity.class);
                ...
                if (value != null) {
                    for (DocumentSnapshot snapshot : value) {
                        ...
                        if (userId.equals(Integer.toString(us.getId()))) {
                            Log.d("softa", "currentUser löydetty");
                            currentUser = us;
                            break;
                        }
                    }
    ...
    // Function to get amount of works from cloud
    private void amountOfWorks() {
        nextWorkId = 0;
        mFirestore.collection("works").addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
            public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                if (value != null) {
                    nextWorkId = value.size() + 1;
                }
            }
        });
    }
